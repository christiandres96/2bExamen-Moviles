...
class MateriaClienteAdapter(private val pokemonList: List<Materia>) :  RecyclerView.Adapter<MateriaClienteAdapter.MyViewHolder>(){
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        val materiaN = pokemonList[position]
        holder.nombre.text = materiaN.nombre
        holder.poderUno.text = materiaN.codigo
        holder.poderDos.text = materiaN.descripcion
        holder.pokemoOn = materiaN
        holder.detalles.setOnClickListener{
            v: View ->
            val intent = Intent(v.context, DetallesMateriaClienteActivity::class.java)
            intent.putExtra("detallesPokemonCliente", materiaN)
